...
import EmailBlocker
from EmailBlocker import __version__, output, quick_thread, get_settings, set_settings, save_settings
import os
import time
...
class WrappingLabel(tk.Label):
    def __init__(self, container, autowrap=True, maxheight=None, **kwargs):
        ...
        Args:
            ...
            maxheight (int): the maximum height the widget can be
    ...
    def config(self, *args, **kwargs):
        '''
        The method for configuring the widget.
        If 'maxheight' is in the kwargs it is dealt with separately.
        If 'text' or 'font' is in the kwargs then `self.wrap_text` is called afterwards.

        Args:
            args (tuple): passed to the `Label` inherited config method
            kwargs (dict): passed to the `Label` inherited config method
        '''
    ...
    def destroy(self, *args, **kwargs):
        ...
        Args:
            args (tuple): passed to `super().destroy`
            kwargs (dict): passed to `super().destroy`
        '''
...
class ScrollableFrame(tk.Frame):
    def __init__(self, container, *args, autobind=True, **kwargs):
        '''
        A *mostly* tkinter compatible Frame-like widget with a scrollbar

        Args:
            container: the parent widget tto spawn this widget on
            args (tuple): passed directly to the `Frame` init
            autobind (bool): whether to bind all children of this widget's scroll events
            kwargs (dict): passed directly to the `Frame` init
        '''
    ...
    def scroll(self, amount):
        '''
        Scrolls the widget by `amount`

        Args:
            amount (tk.Event, int): Either an integer amount to scroll to a tkinter scroll event
        '''
    ...
    def children_bind_scroll(self):
        '''
        Binds all children of this widget's scroll events to `self.scroll`
        '''
    ...
    def children_unbind_scroll(self):
        '''
        Unbinds all children of this widget's scroll events from `self.scroll`
        '''
        children = self.__get_all_children()
        children.append(self.__canvas)
        if os.name == 'nt':
...
class FilterManager(tk.Frame):
    def __init__(self, container, *args, **kwargs):
        ...
        for i in (
            ...
            self.top_bar.append(
                tk.Label(
                    self, text=i, bg='#808080', anchor='nw', height=2
                )
            )
    ...
    def _create(self, item, focus=False, sub=False):
        ...
        ).grid(row=row, column=a, sticky='nesw')
    ...
    def add_row(self, value=None, sub=False):
        if value is not None:
            ...
            try:
                ...
                for r in candidates:
                    row = self.get_row(r)
                    if row is not False and row['search'] == '':
    ...
    def get_row(self, row):
        ...
        for widget in self.grid_slaves(row=row):
            if type(widget) == tk.Entry and 'label' not in widget._name:
                ...
                if widget.grid_info()['row'] % 10 == 0:
                    ...
                    for i in range(1, 10):
                        ...
                        if tmp is not False and 'sub_filters' not in tmp.keys():
            ...
            elif 'checkbutton' in widget._name:
    ...
    def __init__(self):
        ...
        tk.Button(
            ...
            command=lambda: [
                ...
                tmp.children_bind_scroll()
            ],
            relief='groove'
        ).pack(**dk)
        tk.Button(
            self.blocking_frame,
            text='Add blocking sub-rule',
            command=lambda: [
                self.filter_manager.add_row(sub=True),
                tmp.children_bind_scroll()
            ],
            relief='groove'
        ).pack(**dk)
        ...
        self.load_settings_check = tk.Checkbutton(
            self.actions_frame,
            text='Load your saved settings on launch',
            variable=self.load_settings_at_launch,
            command=lambda: self.save_settings(1)
        )
        self.load_settings_check.pack(pady=(0, 10), **dk)
        WrappingLabel(
            self.actions_frame,
            text='NOTICE: I offer no warranty of any kind with this program',
            bg='red'
        ).pack(**dk)
        self.run_button = tk.Button(
            self.actions_frame,
            text='Run',
            command=lambda: quick_thread(self.run),
            relief='groove'
        )
        self.run_button.pack(pady=(5, 10), **dk)

        if os.name == 'nt':
            self.run_at_startup_buton1 = tk.Button(
                self.actions_frame,
                text='Run the current config when the computer starts',
                command=lambda: quick_thread(self.StartupTask_create),
                relief='groove'
            )
            self.run_at_startup_buton1.pack(pady=5, **dk)
            self.run_at_startup_buton2 = tk.Button(
                self.actions_frame,
                text='Remove startup tasks',
                command=lambda: quick_thread(self.StartupTask_destroy),
                relief='groove'
            )
...
def scrollable_popup_yn(msg, title=''):
    ...
    if type(msg) == str:
        msg = [msg]
    for m in msg:
        WrappingLabel(s, text=m, anchor='w', justify='left').pack(side='top', fill='both', expand=True)
    tmp = tk.Frame(p)
    ...
    while vars.yes is False and vars.no is False:
        time.sleep(0.1)
    p.destroy()
    return vars.yes
